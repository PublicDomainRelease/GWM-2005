...
C
 ...
 5000 FORMAT(1X,/1X,'STREAMFLOW CONSTRAINTS:',/,T43,'RIGHT-HAND',
     ...
     3  '-------------------------------------------------------')
 5010 FORMAT(1X,I5,T11,A10,T21,I6,T30,I4,T37,A1,T42,ES11.4,T56,I5)
...
C-----WRITE STATUS FOR A FLOW PROCESS SIMULATION
      IF(IFLG.EQ.1.OR.IFLG.EQ.4)THEN            
        DO 100 ISTC=1,STCNUM
          ...
          DIRR = STCDIR(ISTC)           
          IF(IFLG.EQ.1)THEN
            CALL GWM1BAS3CS(CTYPE,STCNAME(ISTC),0D0,0D0,DIFF,DIRR,1)
      ...
      ELSEIF(IFLG.EQ.2)THEN
        ...
        ISTC = ISTC + 1
          IF(ABS(RHS(ROW)).GT.ZERO)THEN          ! DUAL VARIABLE IS NON-ZERO 
            ...
            CALL SGWM1STC3OT
            CALL GWM1BAS3CS(CTYPE,STCNAME(ISTC),0D0,0D0,RHS(ROW),0,0)
          ENDIF
  200   ENDDO
        RSTRT = RSTRT+STCNUM                     ! SET NEXT STARTING LOCATION
      ENDIF
