...
C-----VARIABLES USED TO CREATE TEMPORARY LP IN GWM1PSIMPLEX1
     ...
     1                           TBNDS(:),TRHS(:)
      INTEGER(I4B),SAVE,ALLOCATABLE::ROWCHK(:)
C
...
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ONE,ZERO,BIGINF,GWMOUT
      USE GWM1BAS2, ONLY : GWM1BAS2PS
...
C-----CHECK THAT NO BOUNDS EXCEED BIGINF, THE VALUE OF MACHINE INFINITY 
      DO 100 I=1,NV
        IF(BNDS(I).GT.BIGINF)THEN
          CALL USTOP('INPUT ERROR, BOUNDS EXCEED BIGINF')
        ENDIF
  100 ENDDO
C
...
C-------SOLUTION FOUND TO PHASE 1.
        IF(IPRT.EQ.1)CALL GWM1BAS2PS('    Feasible Solution Found',0)   
C
...
C-----TEST INPUT ON NUMBER OF VARIABLES
      IF((NDVAR + NSLACK).NE. NV) THEN
        CALL USTOP('INPUT ERROR, NV DOES NOT MATCH NDVAR+NSLACK')
      ENDIF
C
...
C-----TEST FOR ITERATION LIMIT
      IF(IT.GT.LPITMAX)THEN
        CALL USTOP('PROGRAM STOPPED: LPITMAX REACHED')
      ENDIF
	IF(MOD(IT,100).EQ.0)THEN  ! PRINT MESSAGE TO SCREEN 
	  IF(IPRT.EQ.1)CALL GWM1BAS2PS('      Begin LP Iteration',IT)   
      ENDIF
C
...
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO,SMALLEPS
...
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO
      USE GWM1DCV2, ONLY : NBVAR
      USE GWM1RMS2, ONLY : CSTROR,NONLIN,RANGEFLG,LASTLP
      INTEGER(I4B),INTENT(IN)::M,NV
...
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ONE,ZERO,SMALLEPS,BIGINF 
      USE GWM1RMS2, ONLY : CSTREL,CSTRLL,CSTREU,CSTRLU,CSTRLB,CSTRUB,
     &                     CSTROR
      INTEGER(I4B),INTENT(IN)::M,NV
...
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ONE,ZERO,SMALLEPS,BIGINF
      USE GWM1RMS2, ONLY : CSTROR,RHSREL,RHSRLL,RHSREU,RHSRLU,RHSRLB,
     1                     RHSRUB,RHSROR   
      INTEGER(I4B),INTENT(IN)::M,NV
...
C
C-----------------------------------------------------------------------
      USE GWM1DCV2, ONLY : NFVAR,NBVAR,FVON,BVLIST,BVNLIST
      USE GWM1BAS2, ONLY : GWMOUT,BIGINF,ZERO
      USE GWM1RMS2, ONLY : BBITMAX,LPITMAX,BBITPRT,LASTLP
...
C-----------------------------------------------------------------------
      USE GWM1DCV2, ONLY : NFVAR,NBVAR,BVLIST,BVNLIST
      USE GWM1BAS2, ONLY : ZERO,BIGINF,GWMOUT
      USE GWM1RMS2, ONLY : BBITMAX,BBITPRT
...
C---------------------------------------------------------------------------
      USE GWM1DCV2, ONLY : NBVAR
      USE GWM1BAS2, ONLY : GWMOUT,ONE
      USE GWM1RMS2, ONLY : BBITMAX
...
C-----TEST THAT STORAGE SPACE IS AVAILABLE FOR NEXT SUBPROBLEM
      IF(J+2.GT.BBITMAX)
     &          CALL USTOP('STOP: BRANCH AND BOUND ITERATIONS EXCEEDED')
C
...
C-----------------------------------------------------------------------
      ...
      USE GWM1BAS2, ONLY : ZERO,GWMOUT
      USE GWM1BAS2, ONLY : GWM1BAS2PF
...
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      ENDIF
C
      CALL GWM1BAS2PF('    Objective Function =',1,OBJ)
      CALL GWM1BAS2PF(' ',0,ZERO)
C
...
C---------------------------------------------------------------------------
      USE GWM1DCC2, ONLY : NLBR
      USE GWM1DCV2, ONLY : NFVAR,NEVAR,NBVAR,BVNLIST,BVLIST
      USE GWM1BAS2, ONLY : ZERO,ONE,BIGINF
      USE GWM1RMS2, ONLY : BBITMAX,NCONF,NVF,
...
C-----COUNT THE NUMBER OF BINARY VARIABLES THAT ARE TO BE DROPPED
      NDROP = 0
      DO 100 I=1,NBVAR
        IF(IBIN(I,J).GE.0)NDROP = NDROP + 1
  100 ENDDO
C
...
C-----LOOP OVER BINARY VARIABLES SKIPPING THOSE THAT ARE SET
      ...
      DO 210 I=NCVAR+1,NCVAR+NBVAR
        IF(IBIN(I-NCVAR,J).LT.0)THEN        ! BINARY VARIABLE STAYS IN PROBLEM
          TCST(K)=CST(I)
          TBNDS(K)=BNDS(I)
          K = K + 1
        ENDIF
...
C-----LOOP OVER SLACKS FOR -Q + MX >=0 CONSTRAINTS, SKIP SET CONSTRAINTS
      DO 225 I=NV-NBVAR + 1,NV
        IF(IBIN(I-(NV-NBVAR),J).LT.0)THEN
...
C-------CONSIDER COLUMNS WITH BINARY VARIABLES
        ...
        DO 310 K=NCVAR+1,NCVAR+NBVAR
          IF(IBIN(K-NCVAR,J).LT.0)THEN
C---------BINARY IS ACTIVE IN TEMPORARY PROBLEM
            TAMAT(I,KK) = AMAT(I,K)
            KK = KK + 1
          ELSEIF(IBIN(K-NCVAR,J).EQ.0)THEN
C---------BINARY IS TURNED OFF, DON'T DO ANYTHING
          ELSEIF(IBIN(K-NCVAR,J).EQ.1)THEN
...
C-----SECOND, CONSIDER LAST NBVAR ROWS WHICH CONTAIN -Q + MX <=0 CONSTRAINTS
      ...
      DO 360 I=M-NBVAR + 1,M
        INUM = I - (M-NBVAR)
        IF(IBIN(INUM,J).LT.0)THEN           ! BINARY VARIABLE STAYS IN PROBLEM
...
C
      ...
      IMPLICIT NONE
      PRIVATE
      PUBLIC::GWM1RMS2AR,GWM1RMS2PL,GWM1RMS2PP,GWM1RMS2FP,GWM1RMS2FM,
     1        GWM1RMS2AP,GWM1RMS2MPS,GWM1RMS2SLP,GWM1RMS2LP_CHKSOL,
     2        GWM1RMS2OT,GWM1RMS2OS
C
      INTEGER, PARAMETER :: I4B = SELECTED_INT_KIND(9)
      INTEGER, PARAMETER :: I2B = SELECTED_INT_KIND(4)
      INTEGER, PARAMETER :: SP = KIND(1.0)
...
C
      CONTAINS
C***********************************************************************
      SUBROUTINE GWM1RMS2AR(FNAME,IOUT,NFVAR,NEVAR,NBVAR,NDV,NV,NCON)
C***********************************************************************
C   VERSION: 26JULY2006
C   PURPOSE: READ INPUT FROM THE SOLUTION AND OUTPUT-CONTROL FILE
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO,RMFILE,RMFILEF,MPSFILE
      USE GWM1DCV2, ONLY : FVINI,FVDIR,FVNAME,FVBASE,EVNAME,EVBASE,
     &                     BVNAME,BVBASE
      USE GWM1OBJ2, ONLY : SOLNTYP    
      USE GWM1DCC2, ONLY : NLBR
      USE GWM1RMS2, ONLY : SLPVCRIT,SLPZCRIT,BBITPRT,LPITMAX,NONLIN,
     1                     SLPITPRT,SLPITMAX,BBITMAX,NSIGDIG,NPGNMX,
     2                     AFACT,PGFACT,NINFMX,DELINC,FVOLD,
...
C
C-----IF SOLNTYP IS NS (NO SOLUTION) THEN SET IRM=1 AND READ THE NAME
C      OF THE FILE TO WHICH THE RESPONSE MATRIX WILL BE WRITTEN
      IF(SOLNTYP.EQ.'NS')THEN
        ...
        READ(LOCAT,*,ERR=991)NSIGDIG,NPGNMX,PGFACT
        ...
        FUNIT=RMFILE; FMTARG='UNFORMATTED' 
...
C-------PROCESS IRM, THE RESPONSE MATRIX CALCULATION FLAG
		...
		LLOC=1        
          ...
          RMNAME=LINE(INMS:INMF)
          RMFILE=IGETUNIT(7,95)
          OPEN(RMFILE,FILE=RMNAME,STATUS='NEW',FORM=FORM,ERR=998)
          WRITE(IOUT,3010,ERR=990) RMFILE,RMNAME
...
C-------PROCESS LPITMAX AND BBITMAX, MAXIMUM LP AND BRANCH AND BOUND ITERATIONS
        ...
        IF(NBVAR.GT.0.AND.BBITMAX.LT.2)CALL USTOP('BBITMAX MUST BE >1')
C
C-------PROCESS DELTA, THE PERTURBATION SIZE
        IF(ABS(DELTA).GT.50. .OR. ABS(DELTA).LT.5.0D-5)
     &                                          WRITE(IOUT,3020,ERR=990)
...
C----IF SOLNTYP IS SLP 
      IF(SOLNTYP.EQ.'SLP')THEN
        ...
        READ(LOCAT,*,ERR=991)NSIGDIG,NPGNMX,PGFACT,AFACT,NINFMX
...
C
 ...
 3010 FORMAT(1X,/1X,'OPENING RESPONSE-MATRIX FILE FOR SAVING ON UNIT '
     1  ,I4,':',/1X,A200)
...
C-----FILE-OPENING ERROR
 ...
 9990 FORMAT(/,1X,'*** ERROR OPENING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &2X,'-- STOP EXECUTION (GWM1RMS2AR)')
      CALL USTOP(' ')
C
      END SUBROUTINE GWM1RMS2AR
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS2PL(IPERT,NPERT,FIRSTSIM,LASTSIM)
C***********************************************************************
C  VERSION: 25DEC2006
C  PURPOSE - SET THE TERMINAL VALUE OF THE PERTURBATION LOOP
C            AND READ THE RESPONSE MATRIX FROM FILE IF AVAILABLE 
C-----------------------------------------------------------------------
      USE GWM1RMS2, ONLY : SLPITCNT,SLPITMAX,SLPINFCNT,IBASE,IREF,
     &                     OBJOLD,MFCNVRG,DELTA,DELINC,NRMC,NCON,NDV,
     &                     IRM,NRESET
      USE GWM1DCV2, ONLY : NFVAR,NBVAR,NEVAR,FVBASE,FVMAX,FVDIR,FVINI
      USE GWM1HDC2, ONLY : HDCRHS,HDCSTATE0,HDCNAME,HDCDIR
      USE GWM1OBJ2, ONLY : SOLNTYP
      USE GWM1BAS2, ONLY : ZERO,BIGINF,RMFILE,GWMOUT
...
C-------THIS IS THE FIRST TIME THROUGH THIS SUBROUTINE
        MFCNVRG = .TRUE.                         ! INITIALIZE MODFLOW CONVERGENCE
...
C-------THIS IS AN ITERATION OF THE SLP ALGORITHM
        IF(SLPINFCNT.EQ.0)THEN                   ! PRIOR ITERATION SUCCEEDED  
          CALL GWM1RMS2SLP_SETBASE               ! SET NEW BASE SOLUTION AND
          CALL GWM1RMS2SLP_SETDEL                ! SET NEW PERTUBATION VALUE
        ELSEIF(SLPINFCNT.GT.0)THEN               ! PRIOR ITERATION FAILED 
          CALL GWM1RMS2SLP_SETDEL                ! SET ONLY PERTUBATION VALUE
        ENDIF
...
C
      CONTAINS
C***********************************************************************
      SUBROUTINE SGWM1RMS2PL
C***********************************************************************
C
C  PURPOSE: READ THE REFERENCE AND BASE STATES AND RESPONSE MATRIX FROM FILE 
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : GWM1BAS2PS,GWM1BAS2PF
      USE GWM1HDC2, ONLY : GWM1HDC2FPR 
      USE GWM1STC2, ONLY : GWM1STC2FPR
      INTERFACE 
        SUBROUTINE USTOP(STOPMESS)
        CHARACTER STOPMESS*(*)
        END
...
C-----FILE-READING ERROR
 ...
 9910 FORMAT(/,1X,'*** ERROR READING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &2X,'-- STOP EXECUTION (SGWM1RMS2PL)')
      CALL USTOP(' ')
C
      END SUBROUTINE SGWM1RMS2PL
C
C***********************************************************************
      SUBROUTINE GWM1RMS2SLP_SETDEL
C***********************************************************************
      USE GWM1RMS2, ONLY : DINIT,DMIN,DSC
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
...
C
      RETURN
      END SUBROUTINE GWM1RMS2SLP_SETDEL
C
C***********************************************************************
      SUBROUTINE GWM1RMS2SLP_SETBASE 
C***********************************************************************
      USE GWM1RMS2, ONLY : CST,FVOLD
      USE GWM1DCV2, ONLY : NFVAR,FVBASE,NEVAR,EVBASE,NBVAR,BVBASE
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
...
C
      RETURN
      END SUBROUTINE GWM1RMS2SLP_SETBASE
C
      END SUBROUTINE GWM1RMS2PL
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS2PP(IOUT,MFCNVRG,IPERT,FIRSTSIM,LASTSIM)
C***********************************************************************
C  VERSION: 12AUG2006
C  PURPOSE - SET OUTPUT FLAGS AT BEGINNING OF SIMULATION,
C            PERTURB THE PUMPING RATE FOR RESPONSE MATRIX GENERATION
C-----------------------------------------------------------------------
      USE GWM1RMS2, ONLY : NONLIN,NCON,NDV,FVOLD,AFACT,DEWATER,IPGNA,
     1                     DELINC,SLPITCNT,VARBASE,PGFACT,IBASE,IREF,
     2                     DEWATERQ,NPGNMX
      USE GWM1DCV2, ONLY : NFVAR,FVBASE,FVINI,FVMAX,FVNAME
      USE GWM1BAS2, ONLY : ZERO,GWMOUT
      USE GWM1OBJ2, ONLY : SOLNTYP
      USE GWM1BAS2, ONLY : GWM1BAS2PF,GWM1BAS2PS
      INTEGER(I4B),INTENT(IN)::IOUT,IPERT
...
C-------THIS IS A PERTURBATION SIMULATION
        ...
        IF(IPGNA.GT.0)THEN                       ! PERTURBATION NOT SUCCESSFUL
          CALL GWM1RMS2PP_RESETDEL               ! RESET THE PERTURBATION
          DEWATER = .FALSE.                      ! RESET DEWATER FLAG
          DEWATERQ= .FALSE.                      ! INITIALIZE DEWATERQ FLAG
        ENDIF
...
C
C***********************************************************************
      SUBROUTINE SGWM1RMS2PP(IFLG)
C***********************************************************************
C
C  PURPOSE - WRITE OUTPUT FOR GWM1RMS2PP
      INTEGER(I4B)::IFLG
C-----------------------------------------------------------------------
C
      IF(IFLG.EQ.1)THEN
     ...
     &'---------------------------------------------------------------'
     &                    ,0,ZERO)
        CALL GWM1BAS2PS('  Begin Solution Algorithm',0)
        IF(IPERT.LT.0)THEN                       ! THIS IS A REFERENCE RUN
          CALL GWM1BAS2PS('    Running Reference Simulation',0)
     ...
     &      'REFERENCE FLOW PROCESS SIMULATION FOR GWM'
        ELSEIF(IPERT.EQ.0 .AND. IREF.EQ.1)THEN   ! THIS IS A BASE AND REF RUN
          CALL GWM1BAS2PS('    Running Flow Process Simulation',0)
          CALL GWM1BAS2PS('      for both Reference and Base ',0)
          WRITE(IOUT,1005,ERR=990)
     &     'FLOW PROCESS SIMULATION FOR GWM FOR BOTH REFERENCE AND BASE'
        ELSEIF(IPERT.EQ.0 .AND. IREF.EQ.0)THEN   ! THIS IS JUST A BASE RUN
          CALL GWM1BAS2PS('    Running Base Flow Process Simulation',0)
          WRITE(IOUT,1005,ERR=990)'BASE FLOW PROCESS SIMULATION FOR GWM'
        ENDIF
      ELSEIF(IFLG.EQ.2)THEN
        IF(NONLIN)CALL GWM1BAS2PS(' ',0)
        IF(NONLIN)
     &    CALL GWM1BAS2PS('  SLP Algorithm: Begin Iteration ',
     &                  SLPITCNT+1)
        CALL GWM1BAS2PS('    Running Base Flow Process Simulation',0)
        WRITE(IOUT,1005,ERR=990)
     &                            'BASE FLOW PROCESS SIMULATION FOR GWM'
      ELSEIF(IFLG.EQ.3)THEN
        CALL GWM1BAS2PS('      Flow Process Failed: Reset Base',0)
      ELSEIF(IFLG.EQ.4)THEN
        CALL GWM1BAS2PF(
     &'---------------------------------------------------------------'
     &                    ,0,ZERO)
        CALL GWM1BAS2PF('         Final Flow Process Simulation',0,ZERO)
        CALL GWM1BAS2PF(
     &'---------------------------------------------------------------'
     &                    ,0,ZERO)
        CALL GWM1BAS2PS('  Running Final Flow Process Simulation',0)
        CALL GWM1BAS2PS('    using Optimal Flow Variable Rates ',0)
        CALL GWM1BAS2PS('    ',0)
        WRITE(IOUT,1005,ERR=990)
     &                      'FINAL FLOW PROCESS SIMULATION FOR GWM'
      ELSEIF(IFLG.EQ.5)THEN
        CALL GWM1BAS2PS('    ',0)
        CALL GWM1BAS2PS('    Calculating Response Matrix',0)
      ELSEIF(IFLG.EQ.6)THEN
        CALL GWM1BAS2PS('      Perturb Flow Variable',IPERT)
        CALL GWM1BAS2PF('       By Perturbation Value:',1,DELINC(IPERT))
...
C-----FILE-WRITING ERROR
 ...
 9900 FORMAT(/,1X,'*** ERROR WRITING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &7X,'SPECIFIED FILE ACCESS: ',A,/
     &7X,'SPECIFIED FILE ACTION: ',A,/
     &2X,'-- STOP EXECUTION (GWM1RMS2PP)')
      CALL USTOP(' ')
C
      END SUBROUTINE SGWM1RMS2PP
C
C***********************************************************************
      SUBROUTINE GWM1RMS2PP_RESETDEL
C***********************************************************************
C  VERSION: 20FEB2005
...
C---------REDUCE PERTURBATION BUT RETAIN SIGN
        ...
        ELSEIF(IPGNA.EQ.2)THEN
C---------INCREASE PERTURBATION BUT RETAIN SIGN
          DELINC(IPERT) = DELINC(IPERT)/PGFACT
        ELSEIF(IPGNA.EQ.3)THEN
C---------REDUCE PERTURBATION AND CHANGE SIGN
          DELINC(IPERT) = -DELINC(IPERT)*PGFACT
        ELSEIF(IPGNA.EQ.4)THEN
C---------INCREASE PERTURBATION AND CHANGE SIGN
          DELINC(IPERT) = -DELINC(IPERT)/PGFACT
        ENDIF
C
      RETURN
      END SUBROUTINE GWM1RMS2PP_RESETDEL
C
C***********************************************************************
      SUBROUTINE GWM1RMS2PP_RESETBASE
C***********************************************************************
C  VERSION: 20FEB2005
...
C
      RETURN
      END SUBROUTINE GWM1RMS2PP_RESETBASE
C
      END SUBROUTINE GWM1RMS2PP
C
C***********************************************************************
      SUBROUTINE GWM1RMS2OS(KPER)
C***********************************************************************
C     VERSION: 10AUG2007
C     PURPOSE: CHECK THAT NO MANAGED WELL CELL HAS BEEN DEWATERED
C-----------------------------------------------------------------------
      USE GWM1RMS2, ONLY : DEWATERQ
	USE GWM1DCV2, ONLY : NFVAR,FVNCELL,FVKLOC,FVILOC,FVJLOC,FVSP
...
C
      RETURN
      END SUBROUTINE GWM1RMS2OS
C
C***********************************************************************
      SUBROUTINE GWM1RMS2FP(MFCNVRG,IPERT,NPERT,FIRSTSIM,LASTSIM)
C***********************************************************************
C  VERSION: 14AUG2006
...
C-----------------------------------------------------------------------
      USE GWM1RMS2, ONLY : IPGNA,IRM,NCON,NDV,IBASE,DEWATER,DEWATERQ,
     1                     HCLOSEG,SLPITCNT,SLPITPRT,DELINC,NPGNMX,
     2                     VARBASE,NRESET,IREF
      USE GWM1DCV2, ONLY : NFVAR,NEVAR,NBVAR,FVBASE
      USE GWM1BAS2, ONLY : ZERO,RMFILE
      USE GWM1OBJ2, ONLY : SOLNTYP
      USE GWM1HDC2, ONLY : GWM1HDC2FP 
      USE GWM1STC2, ONLY : GWM1STC2FP 
      USE GWM1BAS2, ONLY : GWM1BAS2PF
      LOGICAL(LGT),INTENT(IN)::MFCNVRG,FIRSTSIM,LASTSIM
...
C-----IF LAST SIMULATION SKIP TESTS
     ...
     &            .AND..NOT.DEWATERQ)THEN  ! FINAL FLOW PROCESS CONVERGED
          CALL GWM1RMS2OT(3)               ! WRITE CONSTRAINT STATUS
        ELSEIF(MFCNVRG.AND.
     &        (DEWATER.OR.DEWATERQ))THEN   ! FINAL FLOW PROCESS HAS DEWATERING
          CALL GWM1RMS2OT(4)               ! WRITE CONSTRAINTS AND WARNING 
        ELSEIF(.NOT.MFCNVRG)THEN           ! FINAL FLOW PROCESS DID NOT CONVERGE
          CALL GWM1RMS2OT(5)               ! WRITE MESSAGE 
        ENDIF
        RETURN
      ENDIF
C
C-----TEST FOR PROBLEMS IN THE SIMULATION RESULTS
     ...
     &     .OR.DEWATER.OR.DEWATERQ))THEN   ! FAILED TO CONVERGE OR DEWATERED
        ...
        IF(NRESET.GT.NPGNMX.AND.NPGNMX.GT.0)THEN ! TOO MANY ATTEMPTS 
          CALL GWM1BAS2PF('BASE RESET ATTEMPTS EXCEED MAXIMUM'
     &                     ,0,ZERO)
          CALL USTOP('BASE RESET ATTEMPTS EXCEED MAXIMUM') ! STOP PROGRAM
        ENDIF
        RETURN                           
C
...
C
      ELSEIF(IPERT.GT.0)THEN               ! THIS IS A PERTURBATION SIMULATION
        IF(NPGNMX.EQ.0)THEN                ! DO NOT PERFORM FULL TESTS
          IF(.NOT.MFCNVRG)THEN             ! JUST TEST MF CONVERGENCE
            CALL GWM1BAS2PF  
     &       ('FLOW PROCESS FAILED TO CONVERGE ON PERTURBATION RUN'
     &         ,0,ZERO)
            CALL USTOP                     ! STOP THE RUN
     &       ('FLOW PROCESS FAILED TO CONVERGE ON PERTURBATION RUN')
          ENDIF
        ELSEIF(NPGNMX.GT.0)THEN            ! TEST FOR GOOD RESPONSE
          CALL SGWM1RMS2FP                
          IF(IPGNA.EQ.0)THEN               ! PERTURBATION GENERATION SUCCEEDED
            NPGNA = 0                      ! RESET PERTURBATION ATTEMPT COUNTER
C
          ELSEIF(IPGNA.GT.0) THEN          ! PERTURBATION GENERATION FAILED
            NPGNA = NPGNA + 1              ! INCREMENT PERTURBATION COUNTER 
            IF(NPGNA.GT.NPGNMX)THEN        ! TOO MANY ATTEMPTS; STOP PROGRAM 
              CALL GWM1BAS2PF('PERTURBATION ATTEMPTS EXCEED MAXIMUM'
     &                       ,0,ZERO)
              CALL USTOP('PERTURBATION ATTEMPTS EXCEED MAXIMUM')
            ENDIF
            FVBASE(IPERT) = VARBASE          ! RESTORE ORIGINAL FLOW RATE 
...
C-----WRITE OUTPUT FOR BASE SIMULATIONS
      IF(IPERT.EQ.0)THEN                   ! THIS IS A BASE SIMULATION
        IF(SOLNTYP.EQ.'LP' .OR. SLPITCNT.EQ.0 .OR. SLPITPRT.GE.1)THEN
          CALL GWM1RMS2OT(1)               ! WRITE CONSTRAINTS STATUS
        ENDIF
      ENDIF
C
...
C-----FILE-WRITING ERROR
 ...
 9900 FORMAT(/,1X,'*** ERROR WRITING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &7X,'SPECIFIED FILE ACCESS: ',A,/
     &7X,'SPECIFIED FILE ACTION: ',A,/
     &2X,'-- STOP EXECUTION (GWM1RMS2FP)')
      CALL USTOP(' ')
C
      CONTAINS  
C***********************************************************************
      SUBROUTINE SGWM1RMS2FP
C***********************************************************************
C
C   PURPOSE - TEST RESULTS OF PERTURBED SIMULATION FOR GOOD RESPONSE
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO
      USE GWM1RMS2, ONLY : AMAT,DELINC,NRMC,NSIGDIG,DEWATER,DEWATERQ
      USE GWM1HDC2, ONLY : HDCNUM,HDCSTATE,HDCSTATE0
      USE GWM1STC2, ONLY : NSF,NSD
      USE GWM1STC2, ONLY : STCNUM,STCSTATE,STCSTATE0
      USE GWM1HDC2, ONLY : HDCRHS,NHB,NDD,NDF,NGD,HDCDIR
      USE GWM1BAS2, ONLY : GWM1BAS2PS
      INTEGER IPERT
C-----LOCAL VARIABLES
      REAL(DP)::XNUM
...
C-----CHECK IF MODFLOW FAILED TO CONVERGE
      IF(.NOT.MFCNVRG)THEN
        CALL GWM1BAS2PS('        Perturbation Failed: ',0)
        CALL GWM1BAS2PS('          Flow Process Did Not Converge',0)
        IPGNA = 1                                ! SET PERTURBATION INSTRUCTION
        RETURN
      ENDIF
C
C-----CHECK IF RESPONSE COEFFICIENTS HAVE SUFFICIENT PRECISION
  ...
  200   ENDDO
        IF(COLZ.EQ.0.AND.LNUMMAX.LT.NSIGDIG)THEN ! INSUFFICIENT PRECISION
          CALL GWM1BAS2PS('        Perturbation Failed: ',0)
          CALL GWM1BAS2PS('          Response Precision Inadequate',0)
          IPGNA = 2                              ! SET PERTURBATION INSTRUCTION
          RETURN
        ENDIF
      ENDIF 
C
...
C
      RETURN
      END SUBROUTINE SGWM1RMS2FP
C
      END SUBROUTINE GWM1RMS2FP
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS2FM 
C***********************************************************************
C  VERSION: 26JUL2006
C  PURPOSE - ASSEMBLE THE COEFFICIENT MATRIX, COST COEFFICIENT, UPPER BOUNDS
C              AND RIGHT HAND SIDE FOR THE LINEAR PROGRAM
C    INPUT:
C      AMAT    -ON INPUT, AMAT CONTAINS THE RESPONSE MATRIX COMPUTED IN MKJAKE
...
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO
      USE GWM1RMS2, ONLY : IRM,AMAT,DELINC,NRMC,NCON,NDV,HCLOSEG
      USE GWM1DCV2, ONLY : NFVAR,GWM1DCV2FM
      USE GWM1OBJ2, ONLY : GWM1OBJ2FM 
      USE GWM1HDC2, ONLY : GWM1HDC2FM 
      USE GWM1STC2, ONLY : GWM1STC2FM
      USE GWM1SMC2, ONLY : GWM1SMC2FM
      USE GWM1DCC2, ONLY : GWM1DCC2FM
C-----LOCAL VARIABLES
...
C
 ...
 9010 FORMAT((I5,1X,5G14.6):/(6X,5G14.6))
      END SUBROUTINE SGWM1RMS2FM
C
      END SUBROUTINE GWM1RMS2FM   
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS2AP(GWMCNVRG)
C***********************************************************************
C  VERSION: 09AUG2006
...
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO,BIGINF,SMALLEPS
      USE GWM1BAS2, ONLY : GWM1BAS2PS
      USE GWM1RMS2, ONLY : NRMC,NCON, NV, NDV,LPITMAX,AMAT,CST,BNDS,
     &                     RHS,OBJ,SLPITCNT,SLPITMAX,NCONF,NVF
      USE GWM1DCV2, ONLY : NFVAR,NEVAR,NBVAR,FVBASE,FVDIR,FVNAME,
     &                     FVMIN,FVMAX,EVMIN,EVMAX
      USE GWM1OBJ2, ONLY : SOLNTYP,OBJTYP
      USE GWM1RMS2, ONLY : LASTLP
...
C
      IF(SOLNTYP.EQ.'NS')THEN
        CALL GWM1BAS2PS('    ',0)
        CALL GWM1BAS2PS('    Response Matrix File Has Been Written',0)
        CALL USTOP('  ')
      ELSEIF(SOLNTYP.EQ.'MPS')THEN
        CALL GWM1BAS2PS('    ',0)
        CALL GWM1BAS2PS('    Begin Writing MPS File',0)
        CALL GWM1RMS2MPS
        CALL GWM1BAS2PS('    ',0)
        CALL GWM1BAS2PS('    MPS File Has Been Written',0)
        CALL USTOP('  ')
      ELSEIF(SOLNTYP.EQ.'LP')THEN
        LASTLP = .TRUE.
        CALL SGWM1RMS2AP                    ! SOLVE LP
        CALL GWM1RMS2LP_CHKSOL(IFLG,GWMCNVRG)! CHECK STATUS OF SOLUTION
      ELSEIF(SOLNTYP.EQ.'SLP')THEN
        ...
        IF(SLPITCNT.GT.SLPITMAX)THEN        ! TERMINATE PROGRAM IF ITERATIONS EXCEEDED
          CALL USTOP('PROGRAM STOPPED: SLP ITERATIONS EXCEEDED')
        ENDIF
        LASTLP = .TRUE.                     ! THIS MAY BE LAST - DON'T KNOW
        CALL SGWM1RMS2AP                    ! SOLVE LP AT THIS ITERATION
        CALL GWM1RMS2SLP(IFLG,OBJ,GWMCNVRG) ! CHECK STATUS OF SOLUTION
...
C
      CONTAINS
C***********************************************************************
      SUBROUTINE SGWM1RMS2AP
C***********************************************************************
      USE GWM1RMS2LP, ONLY : GWM1SIMPLEX1,BSOLVE
      USE GWM1BAS2, ONLY : GWM1BAS2PS
      INTEGER(I4B)::J,II
      REAL(DP)::LHS
...
C-----CHECK THAT A MATRIX HAS NO REDUNDANT ROWS
      DO 140 I=1,NCON                            ! LOOP OVER ALL ROWS
        DO 120 II=I+1,NCON                       ! COMPARE WITH ALL LOWER ROWS
          J=1                                    ! COMPARE EACH ELEMENT
          DO WHILE(J.LE.NDV .AND. AMAT(I,J).EQ.AMAT(II,J))
            J=J+1
          ENDDO
          IF(J.EQ.NDV+1)THEN                     ! ALL ELEMENTS SHOULD BE IDENTICAL
            CALL GWM1BAS2PS('    ',0)
            CALL GWM1BAS2PS('  WARNING:     CONSTRAINT NUMBER',I)
            CALL GWM1BAS2PS('  IDENTICAL TO CONSTRAINT NUMBER',II)
            CALL GWM1BAS2PS(
...
C-----CALL SOLVER
      CALL GWM1BAS2PS('    ',0)
      CALL GWM1BAS2PS('    Solving Linear Program',0)
...
C
  992 CONTINUE                                   ! ARRAY-ALLOCATING ERROR
      WRITE(*,9920)
 9920 FORMAT(/,'** ERROR ALLOCATING ARRAY RHS1-- STOP IN SGWM1RMS2AP')
      CALL USTOP(' ')
C
  993 CONTINUE                                   ! ARRAY-DEALLOCATING ERROR
      WRITE(*,9930)
 9930 FORMAT(/,'** ERROR DEALLOCATING ARRAY RHS1-- STOP IN SGWM1RMS2AP')
      CALL USTOP(' ')
C
      END SUBROUTINE SGWM1RMS2AP
C
      END SUBROUTINE GWM1RMS2AP
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS2MPS
C***********************************************************************
C     VERSION: 20FEB2005
...
C---------------------------------------------------------------------------
      USE GWM1BAS2, ONLY : MPSFILE,ONE,ZERO
      USE GWM1RMS2, ONLY : NRMC,NCON,NV,NDV,AMAT,CST,BNDS,RHS
      USE GWM1DCV2, ONLY : NFVAR,NEVAR,NBVAR,FVNAME,EVNAME,BVNAME,
...
C-----FILE-WRITING ERROR
 ...
 9900 FORMAT(/,1X,'*** ERROR WRITING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &7X,'SPECIFIED FILE ACCESS: ',A,/
     &7X,'SPECIFIED FILE ACTION: ',A,/
     &2X,'-- STOP EXECUTION (GWM1RMS2MPS)')
      CALL USTOP(' ')
C
      END SUBROUTINE GWM1RMS2MPS
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS2SLP (IFLG,OBJ,GWMCNVRG)
C***********************************************************************
C  VERSION: 20FEB2005
...
C-----------------------------------------------------------------------
      ...
      USE GWM1RMS2, ONLY : CST,SLPVCRIT,SLPZCRIT,SLPINFCNT,SLPITPRT,
...
C-------TEST FOR CONVERGENCE
        ...
        IF(SLPITPRT.GE.1)THEN                    ! WRITE ITERATION STATUS
          CALL GWM1BAS2PS('    Optimal Solution Found',0)
          CALL GWM1BAS2PF('    Objective Value',1,OBJ)
          IF(NFVAR.GT.0)
     &    CALL GWM1BAS2PF('    Maximum Relative Change in Flow Variable'
     &                    ,1,FVDMAX)
          IF(NEVAR.GT.0)
     &    CALL GWM1BAS2PF('    Max Relative External Variable Change'
     &                    ,1,EVDMAX)
          IF(NBVAR.GT.0)
     &    CALL GWM1BAS2PF('    Maximum Binary Variable Change',1,BVDMAX)
          CALL GWM1BAS2PS('  SLP Algorithm: End Iteration',SLPITCNT)
        ENDIF
        IF(GWMCNVRG)THEN                          ! CONVERGENCE ACHIEVED
          IF(SLPITPRT.GE.1)
     &    CALL GWM1BAS2PS('  Iterations have converged',0)
...
C---------TERMINATE 
          CALL GWM1BAS2PS('  SLP has failed too many times',0)
          CALL USTOP(' ')
        ENDIF
      ENDIF
C
      RETURN
      END SUBROUTINE GWM1RMS2SLP  
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS2LP_CHKSOL(IFLG,GWMCNVRG)
C***********************************************************************
C  VERSION: 20FEB2005
...
C-------PROBLEM IS INFEASIBLE
        CALL GWM1BAS2PS('  No Feasible Solution Found',0)
        CALL USTOP(' ')
      ELSEIF(IFLG.EQ.2)THEN
C-------SOLUTION HAS BEEN FOUND 
        CALL GWM1BAS2PS('  Problem is Unbounded',0)
        CALL USTOP(' ')
      ENDIF
C
      RETURN
      END SUBROUTINE GWM1RMS2LP_CHKSOL
C
C
C***********************************************************************
      SUBROUTINE GWM1RMS2OT(IFLG)
C***********************************************************************
C  VERSION: 09AUG2007
C  PURPOSE - WRITE OUTPUT FOR GWM SOLUTION  
C-----------------------------------------------------------------------
      USE GLOBAL  , ONLY : NPER
      USE GWM1BAS2, ONLY : ZERO,ONE,BIGINF,GWMOUT,GWMWFILE
      USE GWM1RMS2, ONLY : CST,RHS,BNDS,NRMC,NCON,NV,NDV,NONLIN,RHSIN,
     ...
     4                     CSTROR,CSTRLB,CSTRUB,CONTYP,SLPITPRT
      USE GWM1DCV2, ONLY : NFVAR,NBVAR,FVBASE,FVSP,FVNCELL,FVKLOC,
     1                     FVILOC,FVJLOC,FVRATIO,FVNAME
      USE GWM1OBJ2, ONLY : OBJTYP,GWM1OBJ2OT
      USE GWM1BAS2, ONLY : GWM1BAS2PF
      USE GWM1HDC2, ONLY : GWM1HDC2OT
      USE GWM1STC2, ONLY : GWM1STC2OT
      USE GWM1SMC2, ONLY : GWM1SMC2OT
      USE GWM1DCC2, ONLY : GWM1DCC2OT
      INTEGER(I4B),INTENT(IN)::IFLG
      INTERFACE 
        SUBROUTINE USTOP(STOPMESS)
        CHARACTER STOPMESS*(*)
...
C-----LOCAL VARIABLES
      ...
      REAL(SP)::Q
      INTEGER(I4B)::I,II,K,KPER,RSTRT,INDEX,NCON2,OFLG
...
C-----ADJUST VALUE OF IFLG AS NECESSARY
     ...
     &    .OR.(IFLG.EQ.0))THEN              ! OR THIS IS CALL AFTER GWMCNVRG=TRUE
	  IF(GWMWFILE.GT.0)THEN               ! A 'WELL' FILE IS REQUESTED
          REWIND(GWMWFILE)                  ! IF ALREADY WRITTEN TO, REWIND
...
C-----WRITE CONSTRAINT STATUS FOR A FLOW PROCESS SIMULATION
      IF(OFLG.EQ.1)THEN     
        CALL GWM1BAS2PF('      Status of Simulation-Based Constraints '
     &       ,0,ZERO)
        CALL GWM1BAS2PF(
...
C
        ...
        CALL GWM1HDC2OT(RSTRT,1)            ! WRITE HEAD CONSTRAINT STATUS
        CALL GWM1STC2OT(RSTRT,1)            ! WRITE STREAM CONSTRAINT STATUS
        CALL GWM1SMC2OT(RSTRT,1)            ! WRITE SUMMATION CONSTRAINT STATUS
...
C-----WRITE LINEAR PROGRAM OUTPUT
	  ...
	  ELSE
        ...
        ENDIF
        CALL GWM1OBJ2OT                     ! WRITE OBJECTIVE FUNCTION INFO
        RSTRT = 1                           ! SET LOCATION FOR NEXT CONSTRAINT
C
C-------WRITE CONSTRAINTS INFORMATION
        CALL GWM1BAS2PF('       BINDING CONSTRAINTS ',0,ZERO)
        CALL GWM1BAS2PF(
     ...
     &       '---------------        ----     ------      ------------'
     &       ,0,ZERO)
        CALL GWM1HDC2OT(RSTRT,2)            ! WRITE HEAD CONSTRAINT INFO
        CALL GWM1STC2OT(RSTRT,2)            ! WRITE STREAM CONSTRAINT INFO
        CALL GWM1SMC2OT(RSTRT,2)            ! WRITE SUMMATION CONSTRAINT INFO
        CALL GWM1DCC2OT(RSTRT)              ! WRITE VARIABLE CONSTRAINT INFO
C
C-------WRITE RANGE ANALYSIS INFORMATION
        IF(RANGEFLG.EQ.1)THEN
          WRITE(GWMOUT,2000,ERR=990)
          CALL GWM1BAS2PF(' ',0,ZERO)
          CALL GWM1BAS2PF('       RANGE ANALYSIS ',0,ZERO)
...
C
        ELSE
          ...
          CALL GWM1BAS2PF(' ',0,ZERO)
          CALL GWM1BAS2PF('       Range Analysis Not Reported ',0,ZERO)
        ENDIF
C
C-----WRITE FINAL CONSTRAINT STATUS
     ...
     &       ,0,ZERO)
        ...
        CALL GWM1HDC2OT(RSTRT,1)                  ! WRITE HEAD CONSTRAINT STATUS
        CALL GWM1STC2OT(RSTRT,1)                  ! WRITE STREAM CONSTRAINT STATUS
        CALL GWM1SMC2OT(RSTRT,3)                  ! WRITE SUMMATION CONSTRAINT STATUS
	  WRITE(GWMOUT,7005,ERR=990)
...
C-----WRITE OPTIMAL SOLUTION WITH A FORMAT SIMILAR TO THE MODFLOW WELL FILE
      IF(OFLG.EQ.6)THEN  
        DO 630 KPER=1,NPER
          II=0                                  ! COUNT THE NUMBER OF WELLS 
          DO 600 I=1,NFVAR                      ! LOOP OVER FLOW VARIABLES
            IF(CST(I).NE.ZERO.AND.FVSP(I,KPER))THEN ! FLOW VARIABLE ACTIVE
              II=II+FVNCELL(I)                  ! COUNT NUMBER OF CELLS
            ENDIF
  600     ENDDO
          WRITE(GWMWFILE,6000)II,0,KPER       ! WRITE THE NUMBER OF WELLS
          IF(II.GT.0)THEN                       ! CHECK THAT YOU HAVE SOME WELLS
            DO 620 I=1,NFVAR                    ! LOOP OVER GWM FLOW VARIABLES
              IF(CST(I).NE.ZERO.AND.FVSP(I,KPER))THEN ! FLOW VARIABLE ACTIVE
                DO 610 K=1,FVNCELL(I)           ! LOOP OVER CELLS FOR THIS VARIABLE
                  Q=REAL(CST(I),KIND=4)*FVRATIO(I,K)! ASSIGN FLOW RATE 
                  WRITE(GWMWFILE,6010)FVKLOC(I,K),FVILOC(I,K),
     &                                FVJLOC(I,K),Q,FVNAME(I),K
  610           ENDDO
              ENDIF
  620       ENDDO
          ENDIF
...
C
      END SUBROUTINE GWM1RMS2OT
C
C
      END MODULE GWM1RMS2SUBS
C
C
C
C
