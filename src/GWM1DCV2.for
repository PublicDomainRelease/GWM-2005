...
C     VERSION: 20FEB2005
     ...
     &        EVBASE,EVDIR,NEVAR,BVNAME,BVBASE,BVNLIST,BVLIST,NBVAR
      PUBLIC::GWM1DCV2AR,GWM1DCV2FM,GWF2DCV2FM,GWF2DCV2BD
C
...
C
      CONTAINS 
C*************************************************************************
      SUBROUTINE GWM1DCV2AR(FNAME,IOUT,NROW,NCOL,NLAY,NPER,
     &                      NFVAR,NEVAR,NBVAR,NDV)
C*************************************************************************
C     VERSION: 11AUG2008
C     PURPOSE: READ INPUT FROM THE DECISION-VARIABLE FILE 
C---------------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ONE,ZERO,GWMWFILE,GWM1BAS2PS,CUTCOM
...
C-----READ NFVAR, NEVAR, AND NBVAR
      ...
      IF(NFVAR.LE.0)THEN
        CALL GWM1BAS2PS(' PROGRAM STOPPED: NFVAR MUST BE > 0',0)
        CALL USTOP(' ')
      ENDIF
...
C
      ...
      END SUBROUTINE GWM1DCV2AR
C
C
C***********************************************************************
      SUBROUTINE GWM1DCV2FM
C***********************************************************************
C     VERSION: 20FEB2005
C     PURPOSE: FORMULATE THE BOUNDS ON DECISION VARIABLES
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ONE,SMALLEPS
      USE GWM1RMS2, ONLY : BNDS
C-----LOCAL VARIABLES
      INTEGER(I4B)::I
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C
...
C
      RETURN
      END SUBROUTINE GWM1DCV2FM
C
C
C***********************************************************************
      SUBROUTINE GWF2DCV2FM(KPER)
C***********************************************************************
C     VERSION: 10AUG2007
...
C
      RETURN
      END SUBROUTINE GWF2DCV2FM
C
C***********************************************************************
      SUBROUTINE GWF2DCV2BD(KSTP,KPER)
C***********************************************************************
C     VERSION: 10AUG2007
...
C-----------------------------------------------------------------------
      ...
      INTEGER(I4B),INTENT(IN)::KSTP,KPER
C-----LOCAL VARIABLES
      INTEGER(I4B)::I,K,IL,IR,IC
      REAL(SP)::Q 
