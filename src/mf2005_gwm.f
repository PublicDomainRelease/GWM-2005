...
C     ******************************************************************
C       MODIFIED TO OPERATE WITH THE GROUND-WATER MANAGEMENT PROCESS - GWM
C       CODE INSERTIONS MARKED WITH COMMENT LINE "C GWM:"
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C GWM: GWM SUBROUTINES CALLED FROM MF2005 MAIN PROGRAM
     ...
     &                        GWM1BAS2DABAS7
      USE GWM1RMS2SUBS, ONLY: GWM1RMS2PL,GWM1RMS2PP,GWM1RMS2FP, 
     &                        GWM1RMS2FM,GWM1RMS2AP,GWM1RMS2OT,
     &                        GWM1RMS2OS   
      USE GWM1DCV2, ONLY: GWF2DCV2FM,GWF2DCV2BD 
      USE GWM1HDC2, ONLY: GWM1HDC2OS 
      USE GWM1STC2, ONLY: GWM1STC2OS,GWM1STC2OS2
      INCLUDE 'openspec.inc'
C
C-------ASSIGN VERSION NUMBER AND DATE
      CHARACTER*40 VERSION
      CHARACTER*10 MFVNAM
      PARAMETER (VERSION='MF2005_GWM 1.1 061209,FROM MF05V1.6.02')
...
C GWM: READ GWM PROCESS INFORMATION
     ...
     &                    NPER,PERLEN,HCLOSET,VERSION,MFVNAM)
      ENDIF
C
...
C-------PREPARE PERTURBATION LOOP CONTROLS
        ...
        ELSEIF(IUNIT(56).GT.0)THEN     ! GWM ACTIVE; SET LOOP CONTROLS
          CALL GWM1RMS2PL(IPERT,NPERT,FIRSTSIM,LASTSIM)
        ENDIF
C
...
C-------PREPARE CALCULATION OF SIMULATION RESPONSE
        IF(IUNIT(56).GT.0) 
     &      CALL GWM1RMS2PP(IOUT,MFCNVRG,IPERT,FIRSTSIM,LASTSIM)
C
...
C GWM: ADD GWM MANAGED FLOWS
            IF(IUNIT(56).GT.0) CALL GWF2DCV2FM(KKPER)
...
C GWM: IF GWM ACTIVE AND CONVERGENCE HAS FAILED SKIP TO PERTURBATION LOOP END
            IF(IUNIT(56).GT.0)THEN
              MFCNVRG = .FALSE.
              CALL GWM1RMS2FP(MFCNVRG,IPERT,NPERT,FIRSTSIM,LASTSIM)
              FIRSTSIM = .FALSE. 
              GO TO 200
            ENDIF       
C
...
C GWM: RECORD BUDGET FOR GWM MANAGED FLOW VARIABLES
          IF(IUNIT(56).GT.0) CALL GWF2DCV2BD(KKSTP,KKPER)    
...
C GWM: ASSIGN SYSTEM STATE TO STATE ARRAY AT END OF STRESS PERIOD
        IF(IUNIT(56).GT.0)THEN   
          CALL GWM1RMS2OS(KKPER,HDRY) ! CHECK MANAGED WELLS FOR DEWATER
          CALL GWM1HDC2OS(KKPER,HDRY)
          IF(IUNIT(18).GT.0)CALL GWM1STC2OS(KKPER) ! STR PACKAGE
          IF(IUNIT(44).GT.0)CALL GWM1STC2OS2(KKPER)! SFR PACKAGE
        ENDIF
  100 CONTINUE
C
...
C-------IF NOT THE LAST SIMULATION, SOLVE GWM PROBLEM AT THIS ITERATION
        IF(.NOT. LASTSIM)THEN           ! GWM PROCESS MUST BE ACTIVE
          CALL GWM1RMS2FM               ! FORMULATE THE GWM PROBLEM
          CALL GWM1RMS2AP(GWMCNVRG)     ! SOLVE THE GWM PROBLEM
          IF(.NOT.GWMCNVRG)CALL GWM1RMS2OT(-1)! WRITE GWM SOLUTION OUTPUT 
        ENDIF
C
C-------IF GWM PROBLEM HAS CONVERGED, ONE MORE SIMULATION RUN IS NEEDED
        IF(GWMCNVRG .AND. .NOT. LASTSIM)THEN
          CALL GWM1RMS2OT(0)            ! WRITE GWM SOLUTION OUTPUT 
