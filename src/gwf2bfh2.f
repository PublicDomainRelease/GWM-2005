      MODULE GWFBFHMODULE
         ...
         INTEGER,          SAVE, POINTER, DIMENSION(:)     ::IBB
         ...
         REAL,             SAVE, POINTER, DIMENSION(:)     ::BCOND
...
C     ------------------------------------------------------------------
     ...
     2                      NPCEND,NLAYP,NROWP,NCOLP,IUBC,NGRIDS,NCPP,
     3                      BTEXT,IBPFLG,IBB,NPINDX,KLAY,IROW,JCOL,
...
C     ------------------------------------------------------------------
      ALLOCATE(ISCHILD,IBOTFLG,IBFLG,NPBNODES,NCBNODES,NTIMES,NPLBEG,
     &         NPRBEG,NPCBEG,NPLEND,NPREND,NPCEND,NLAYP,NROWP,NCOLP,
     &         IUBC,NGRIDS,NCPP,BTEXT,VCB(4))
      
C1------PRINT A MESSAGE IDENTIFYING BFH PACKAGE
      WRITE(IOUT,500)IN
  500 FORMAT(1X,/1X,'BFH2 -- BOUNDARY FLOW AND HEAD PACKAGE',
     &    ', VERSION 2.0, 06/25/2013',/8X,'INPUT READ FROM UNIT ',I4)
...
C3------NOTE THAT WE ARE SKIPPING NLAY,NROW,NCOL BECAUSE WE HAVE IT ALREADY
     ...
     &   LINE(ISTART1:ISTOP1) .EQ. 'MULTIPLE')THEN
      ...
      ELSEIF(ISCHILD .LE. 0 .AND. NEWFLG .EQ. 1)THEN
        ALLOCATE(IBPFLG(NGRIDS-1),IBB(NCBNODES))
        CALL URWORD(LINE,IC,ISTART,ISTOP,2,IUBC,R,0,0)
        READ(IN,*) (IBPFLG(LG),LG=1,NGRIDS-1)
      ENDIF
C
C3A-----PRINT COMMENTS
...
C3A-----THE INTERIOR CELLS BETWEEN INTERFACE CELLS AND ZERO THEM OUT.
        ...
        DO LG = 1,NGRIDS-1
          NPLBEG=MINVAL(KLAY,IBB.EQ. IBPFLG(LG))
          NPLEND=MAX(1,(MAXVAL(KLAY,IBB.EQ. IBPFLG(LG))-1))
          NPRBEG=MINVAL(IROW,IBB.EQ. IBPFLG(LG))+1
          NPREND=MAXVAL(IROW,IBB.EQ. IBPFLG(LG))-1
          NPCBEG=MINVAL(JCOL,IBB.EQ. IBPFLG(LG))+1
          NPCEND=MAXVAL(JCOL,IBB.EQ. IBPFLG(LG))-1
          DO K = NPLBEG,NPLEND
            DO I = NPRBEG,NPREND
              DO J = NPCBEG,NPCEND
...
C     AND THE COMPLIMENTARY BOUNDARY CONDITION DATA, IF USED
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
