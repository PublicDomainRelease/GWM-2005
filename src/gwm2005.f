...
C-------ASSIGN VERSION NUMBER AND DATE
      ...
      CHARACTER*10 MFVNAM
      PARAMETER (VERSION='GWM2005 1.2 120709,FROM MF-LGR V1.1.02')
...
C-------PREPARE CALCULATION OF SIMULATION RESPONSE
        IF(GWMACTIVE) 
     &      CALL GWM1RMS3PP(IOUT,IPERT,FIRSTSIM,LASTSIM,NPERT)
C
      DO IGRID = 1, NGRIDS
...
C GWM:  ASSIGNS SYSTEM STATE TO STATE ARRAY AT END OF STRESS PERIOD
        IF(GWMACTIVE)THEN   
          DO 95 IGRID=1,NGRIDS
            CALL SGWF2BAS7PNT(IGRID)
            CALL GWM1RMS3OS(KKPER,HDRY,IGRID) ! CHECK MANAGED WELLS FOR DEWATER
