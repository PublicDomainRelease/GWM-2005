...
C GWM: GWM SUBROUTINES CALLED FROM MF2005 MAIN PROGRAM
      USE GWM1BAS3SUBS, ONLY: GWM1BAS3AR,GWM1BAS3RW,GWM1BAS3RRF,
     &                        GWM1BAS3DABAS7,GWM1BAS3AP
      USE GWM1RMS3SUBS, ONLY: GWM1RMS3PL,GWM1RMS3PP,GWM1RMS3FP, 
     &                        GWM1RMS3FM,GWM1RMS3AP,GWM1RMS3OT,
     &                        GWM1RMS3OS 
      USE GWM1DCV3, ONLY: GWF2DCV3FM,GWF2DCV3BD 
      USE GWM1HDC3, ONLY: GWM1HDC3OS
      USE GWM1STC3, ONLY: GWM1STC3OS,GWM1STC3OS2
      USE GWM1STA3, ONLY: GWM1STA3OS 
      USE GWM1RMS3, ONLY: MFCNVRG
      INCLUDE 'openspec.inc'
C
C-------ASSIGN VERSION NUMBER AND DATE
      CHARACTER*40 VERSION
      CHARACTER*10 MFVNAM
      PARAMETER (VERSION='GWM-2005 1.4.1 062012,FROM MF-LGR V1.2')
      PARAMETER (MFVNAM='-LGR')
C
      CHARACTER*80 HEADNG(2)
      CHARACTER*200 FNAME
      INTEGER IBDT(8)
C GWM: LOCAL VARIABLES CREATED FOR GWM
      LOGICAL FIRSTSIM,LASTSIM,FINISH,GWMCNVRG,GWMACTIVE
      INTEGER IPERT,NPERT 
      REAL HCLOSET,DUMHCLOSE   
      DOUBLE PRECISION, DIMENSION(1):: DEPVALS  ! DUMMY ARRAY NEEDED FOR GWMPLL
      CHARACTER(LEN=200),ALLOCATABLE::FNAMEV(:)
...
C3------GET THE NAME OF THE NAME FILE
      ...
      ALLOCATE(FNAMEV(NGRIDS))                   ! GWM: CREATE STORAGE
      GWMACTIVE=.FALSE.                          ! INITIALIZE NO GWM FILES
      DUMHCLOSE = 100000.0                       ! INITIALIZE FOR COMPARISON
      DO IGRID = 1, NGRIDS
        MAXUNIT= INUNIT
C
C4A------IF USING LGR, READ NAMES FROM LGR NAME FILE
        IF(ILGR .NE. 0) CALL GETNAMFILLGR(ILUNIT,FNAME,IGRID)
        FNAMEV(IGRID)=FNAME                      ! GWM SAVE MF NAME FILES
C4B-----OPEN NAME FILE.
...
c       FIND SMALLEST HCLOSE VALUE ACROSS GRIDS
        IF(IUNIT(9).GT.0)  DUMHCLOSE = MIN(DUMHCLOSE,HCLOSE)    !SIP
        IF(IUNIT(10).GT.0) DUMHCLOSE = MIN(DUMHCLOSE,HCLOSEDE4) !DE4
        IF(IUNIT(13).GT.0) DUMHCLOSE = MIN(DUMHCLOSE,HCLOSEPCG) !PCG
        IF(IUNIT(42).GT.0) DUMHCLOSE = MIN(DUMHCLOSE,HCLOSEGMG) !GMG
C  Observation allocate and read
        ...
        IF(IUNIT(56).GT.0)GWMACTIVE=.TRUE.       ! TRUE IF GWM ON ANY GRID
C END LOOP FOR ALLOCATING AND READING DATA FOR EACH GRID
      ENDDO
C GWM: READ GWM PROCESS INFORMATION
      IF(GWMACTIVE)THEN						
        CALL SGWF2BAS7PNT(1)                ! POINT TO PARENT FOR NPER, PERLEN
        CALL GWM1BAS3AR(IOUT,NPER,PERLEN,DUMHCLOSE,VERSION,MFVNAM,
     &                  NGRIDS)
      ENDIF
C
...
C-----PREPARE ITERATION LOOP CONTROLS
      IF(.NOT.GWMACTIVE)THEN      ! GWM NOT ACTIVE; LOOP FOR ONE PASS
        LASTSIM = .TRUE.
      ELSEIF(GWMACTIVE)THEN       ! GWM ACTIVE; LOOP FOR GWM ITERATIONS
        LASTSIM = .FALSE.
      ENDIF
C
...
C-------PREPARE PERTURBATION LOOP CONTROLS
        IF(.NOT.GWMACTIVE)THEN    ! GWM NOT ACTIVE; LOOP FOR ONE PASS
          IPERT = 1
          NPERT = 1
        ELSEIF(GWMACTIVE)THEN     ! GWM ACTIVE; SET LOOP CONTROLS
...
C-------PREPARE CALCULATION OF SIMULATION RESPONSE
        IF(GWMACTIVE) 
     &      CALL GWM1RMS3PP(IOUT,IPERT,FIRSTSIM,LASTSIM,NPERT,GWMSTRG)
C
      DO IGRID = 1, NGRIDS
        CALL SGWF2BAS7PNT(IGRID)
        FNAME=FNAMEV(IGRID)
C GWM: PREPARE FLOW PROCESS FILES FOR NEXT SIMULATION
        IF(GWMACTIVE .AND. .NOT.FIRSTSIM)THEN
C---------REWIND FLOW PROCESS MODEL FILES AND RE-READ BAS FILE 
          CALL GWM1BAS3RW(INUNIT,FNAME,CUNIT,NIUNIT,IOUT,
     &                VERSION,MFVNAM,GWMSTRG)
          CALL GWM1BAS3RRF(INUNIT,CUNIT,VERSION,24,31,32,
     &                IGRID,12,HEADNG,26,MFVNAM,FNAME,LASTSIM)
        ENDIF     
...
C GWM : ADD GWM MANAGED FLOWS
                  IF(IUNIT(56).GT.0) CALL GWF2DCV3FM(KKPER,IGRID)
...
C---------END LGR ITERATION LOOP
          ENDDO       
C
C GWM: TEST IF GWF SOLUTION IS ACCEPTABLE USING GWF CRITERIA
          IF(GWMACTIVE)THEN
            CALL GWM1BAS3AP(1,ILGR,ICNVG,LGRCNVG,IPERT,IGRID) 
            IF(.NOT.MFCNVRG(IPERT))THEN       ! SOLUTION NOT ACCEPTABLE TO GWM
              CALL GWM1RMS3FP(IPERT,NPERT,FIRSTSIM,
     &                        LASTSIM)        ! RESET PERTURBATION PARAMETERS
              FIRSTSIM = .FALSE. 
              GO TO 200                       ! SKIP TO PERTURBATION LOOP END
            ENDIF
          ENDIF       
C
...
C GWM: RECORD BUDGET FOR GWM MANAGED FLOW VARIABLES
            IF(IUNIT(56).GT.0)CALL GWF2DCV3BD(KKSTP,KKPER,IGRID)    
...
C
C GWM: TEST IF GWF SOLUTION IS CONVERGED USING WATER BALANCE CRITERIA
            IF(GWMACTIVE)THEN
              CALL GWM1BAS3AP(2,ILGR,ICNVG,LGRCNVG,IPERT,IGRID) 
              IF(.NOT.MFCNVRG(IPERT))THEN       ! SOLUTION NOT ACCEPTABLE TO GWM
                CALL GWM1RMS3FP(IPERT,NPERT,FIRSTSIM,
     &                          LASTSIM)        ! RESET PERTURBATION PARAMETERS
                FIRSTSIM = .FALSE. 
...
C------GWM TAKES CONTROL OF LOOPING HERE
            IF(.NOT.GWMACTIVE.AND.ICNVG.EQ.0 .AND. ILGR .EQ. 0)GO TO 110
C            IF(ICNVG.EQ.0 .AND. ILGR .EQ. 0) GO TO 110
C---------END GRID OT GRID LOOP
          ENDDO
C
C GWM:  ACCUMULATE STORAGE CHANGES FOR STORAGE STATE VARIABLE AT EACH TIME STEP
            IF(GWMACTIVE)THEN   
              DO 85 IGRID=1,NGRIDS
               CALL SGWF2BAS7PNT(IGRID)
               CALL GWM1STA3OS(IGRID,KKPER,IPERT,KSTP,HDRY,0,IUNIT(18),
     &               IUNIT(44),IUNIT(1),IUNIT(23),IUNIT(37),0,0,DEPVALS)
   85         ENDDO
            ENDIF
C
...
C GWM:  ASSIGNS SYSTEM STATE TO STATE ARRAY AT END OF STRESS PERIOD
        IF(GWMACTIVE)THEN   
          DO 95 IGRID=1,NGRIDS
            CALL SGWF2BAS7PNT(IGRID)
            CALL GWM1RMS3OS(IGRID,KKPER,IPERT,HDRY) ! CHECK MANAGED WELLS FOR DEWATER
            CALL GWM1HDC3OS(IGRID,KKPER,IPERT,HDRY,0,DEPVALS)
            IF(IUNIT(18).GT.0)
     &      CALL GWM1STC3OS(IGRID,KKPER,0,0,DEPVALS)  ! STR PACKAGE
            IF(IUNIT(44).GT.0)
     &      CALL GWM1STC3OS2(IGRID,KKPER,0,0,DEPVALS) ! SFR PACKAGE
            CALL GWM1STA3OS(IGRID,KKPER,IPERT,KSTP,HDRY,1,IUNIT(18),
     &              IUNIT(44),IUNIT(1),IUNIT(23),IUNIT(37),0,0,DEPVALS) 
   95     ENDDO
        ENDIF
  100 CONTINUE
C
      IF(.NOT.GWMACTIVE)THEN ! GWM NOT ACTIVE
        IPERT = 2            ! INCREMENT IPERT TO TERMINATE PERTURBATION LOOP
      ELSEIF(GWMACTIVE)THEN ! ANALYZE SIMULATION RESPONSE FOR GWM
        MFCNVRG(IPERT) = .TRUE.  ! SIMULATION HAS CONVERGED
...
C9------LAST BECAUSE IT DEALLOCATES IUNIT.
     ...
     1                                              IGRID)
        ...
        IF(.NOT. LASTSIM)  CALL GWM1BAS3DABAS7(IGRID)!DEALLOCATE SOME BAS7 MEMORY
      ENDDO
C
...
C-------IF NOT THE LAST SIMULATION, SOLVE GWM PROBLEM AT THIS ITERATION
        IF(.NOT. LASTSIM)THEN           ! GWM PROCESS MUST BE ACTIVE
          ...
          IF(.NOT.GWMCNVRG)CALL GWM1RMS3OT(-1,NGRIDS)! WRITE GWM SOLUTION OUTPUT 
        ENDIF
C
C-------IF GWM PROBLEM HAS CONVERGED, ONE MORE SIMULATION RUN IS NEEDED
        IF(GWMCNVRG .AND. .NOT. LASTSIM)THEN
          CALL GWM1RMS3OT(0,NGRIDS)     ! WRITE GWM SOLUTION OUTPUT 
...
C-----END OF GWM ITERATION LOOP
  300 ENDDO
C
C GWM: DEALLOCATE REMAINING PACKAGES: LGR, SOLVER, OBS2 AND BAS7 
C      GWF2BAS7DA MUST BE CALLED LAST BECAUSE IT DEALLOCATES IUNIT.
      DO IGRID = 1, NGRIDS
        CALL SGWF2BAS7PNT(IGRID)
