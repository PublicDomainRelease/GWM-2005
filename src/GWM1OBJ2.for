...
C     VERSION: 12AUG2007
      IMPLICIT NONE
      PRIVATE
      PUBLIC::OBJTYP,SOLNTYP
      PUBLIC::GWM1OBJ2AR,GWM1OBJ2FM,GWM1OBJ2OT
C
...
C
      ...
      INTEGER(I4B),SAVE::NFVOBJ,NEVOBJ,NBVOBJ
      REAL(SP),SAVE,ALLOCATABLE::FVOBJC(:),EVOBJC(:),BVOBJC(:)
C
...
C
      CONTAINS
C***********************************************************************
      SUBROUTINE GWM1OBJ2AR(FNAME,IOUT,NPER,PERLEN,NFVAR,NEVAR,NBVAR)
C***********************************************************************
C     VERSION: 21JULY2005
C     PURPOSE: READ INPUT FROM THE OBJECTIVE-FUNCTION FILE
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO
      USE GWM1DCV2, ONLY : FVNAME,EVNAME,BVNAME,FVSP,EVSP
...
C-----LOCAL VARIABLES
      ...
      REAL(DP)::TTIME
      REAL(SP)::TFVOBJC,TEVOBJC,TBVOBJC
      CHARACTER(LEN=10)::TFVNAME,TEVNAME,TBVNAME
...
C-----READ OBJTYP AND FNTYP AND TEST FOR VALIDITY
      ...
      IF(FNTYP.NE.'WSDV')THEN
        WRITE(IOUT,4000,ERR=990)                 ! FNTYP NAME NOT VALID
        CALL USTOP(' ')
      ENDIF
      WRITE(IOUT,5000,ERR=990)OBJTYP,FNTYP
C
C-----READ NFVOBJ, NEVOBJ, AND NBVOBJ AND TEST FOR VALIDITY
      READ(LOCAT,*,ERR=991)NFVOBJ,NEVOBJ,NBVOBJ
      ...
      IF(NBVOBJ.GT.NBVAR)THEN
        ...
        CALL USTOP(' ')
      ENDIF
      WRITE(IOUT,6030,ERR=990)NFVOBJ,NEVOBJ,NBVOBJ
C
C-----ALLOCATE LOCAL ARRAYS FOR OUTPUT
      ALLOCATE (FVOBJC(NFVAR),EVOBJC(NEVAR),BVOBJC(NBVAR),
     &          PNW(NFVAR+NEVAR+NBVAR),CSW(NFVAR+NEVAR+NBVAR),
     &          PCW(NFVAR+NEVAR+NBVAR),STAT=ISTAT)
      IF(ISTAT.NE.0)GOTO 992 
      BYTES = 4*(NFVAR+NEVAR+NBVAR)
C
...
C-------MULTIPLY COSTS BY DURATION OF STRESS PERIODS
  ...
  210   ENDDO
        FVOBJC(I) = FVOBJC(I) * REAL(TTIME,SP)   ! ADJUST COSTS
  200 ENDDO
C
...
C-------MULTIPLY COSTS BY DURATION OF STRESS PERIODS
  ...
  340   ENDDO
        EVOBJC(I) = EVOBJC(I) * REAL(TTIME,SP)   ! ADJUST COSTS
  360 ENDDO
C
...
C
 ...
 6000 FORMAT(1X,/1X,'PROGRAM STOPPED. NFVOBJ MUST BE LESS THAN OR',
     1  ' EQUAL TO NFVAR.')
 6020 FORMAT(1X,/1X,'PROGRAM STOPPED. NBVOPJ MUST BE LESS THAN OR',
     1  ' EQUAL TO NBVAR.')
...
C
      RETURN
      END SUBROUTINE GWM1OBJ2FM
C
C
C***********************************************************************
      SUBROUTINE GWM1OBJ2OT
C***********************************************************************
C     VERSION: 12AUG2006
C     PURPOSE: WRITE OUTPUT FOR OBJECTIVE FUNCTION
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO,GWMOUT
      USE GWM1DCV2, ONLY : NFVAR,NEVAR,NBVAR,EVDIR,
     1                     FVNAME,FVDIR,EVNAME,BVNAME
      USE GWM1RMS2, ONLY : CST,RANGENAME,RANGENAMEF
      INTERFACE
        SUBROUTINE USTOP(STOPMESS)
        CHARACTER STOPMESS*(*)
        END
...
C-----WRITE OPTIMAL FLOW VARIABLES
      ...
      DO 100 I=1,NFVAR
        IF(FVDIR(I).EQ.1)THEN                    ! THIS IS INJECTION OF WATER
          OBJC = REAL(FVOBJC(I),DP)*CST(I)
          WRITE(GWMOUT,2100,ERR=990) FVNAME(I),CST(I),OBJC
          SUMI = SUMI + CST(I)
        ELSE                                     ! THIS IS WITHDRAWAL OF WATER
          OBJC = -REAL(FVOBJC(I),DP)*CST(I)
          WRITE(GWMOUT,2200,ERR=990) FVNAME(I),-CST(I),OBJC
          SUME = SUME - CST(I)
        ENDIF
        SUMC = SUMC + OBJC
 ...
 100  ENDDO
      WRITE(GWMOUT,3000,ERR=990)SUME,SUMI,SUMC
      SUMCT = SUMC
C
C-----WRITE OPTIMAL EXTERNAL VARIABLES
      ...
      DO 200 I=1,NEVAR
        ICST = ICST + 1
        IF(EVDIR(I).EQ.1)THEN                    ! THIS IS AN IMPORT OF WATER
          OBJC = REAL(EVOBJC(I),DP)*CST(ICST)
          WRITE(GWMOUT,2100,ERR=990) EVNAME(I),CST(ICST),OBJC
          SUMI = SUMI + CST(ICST)
        ELSE                                     ! THIS IS AN EXPORT OF WATER
          OBJC = -REAL(EVOBJC(I),DP)*CST(ICST)
          WRITE(GWMOUT,2200,ERR=990) EVNAME(I),-CST(ICST),OBJC
          SUME = SUME - CST(ICST)
        ENDIF
        SUMC = SUMC + OBJC
...
C-----WRITE OPTIMAL BINARY VARIABLES
      ...
      DO 300 I=1,NBVAR
        ICST = ICST + 1
        WRITE(GWMOUT,6000,ERR=990) BVNAME(I),INT(CST(ICST)),
     &                             BVOBJC(I)*CST(ICST)
        SUMI = SUMI + CST(ICST)
  ...
  300 ENDDO
      WRITE(GWMOUT,7000,ERR=990)INT(SUMI),SUMC
      SUMCT = SUMCT+SUMC
      ENDIF
...
C
 ...
 3000 FORMAT(  T20,12('-'),T40,12('-'),T60,12('-'),
     &       /,T1,'TOTALS',T19,ES13.6,T39,ES13.6,T59,ES13.6,/)
 4000 FORMAT(T8,'OPTIMAL RATES FOR EACH EXTERNAL VARIABLE ',
     &     /,  T8,39('-'),
     &     /,  T1,'Variable',T20,'Export',
     &         T40,'Import',T60,'Contribution',
     &     /,  T1,'Name',T20,'Rate',T40,'Rate',T60,'To Objective',
     &     /,  T1,10('-'),T20,14('-'),T40,12('-'),T60,12('-'))
 5000 FORMAT(T8,'OPTIMAL VALUES FOR EACH BINARY VARIABLE ',
     &     /,  T8,39('-'),
     &     /,  T1,'Variable',T60,'Contribution',
     &     /,  T1,'Name',T20,'Value',T60,'To Objective',
     &     /,  T1,10('-'),T20,12('-'),T60,12('-'))
 6000 FORMAT(1P,T2,A,T20,I5,T59,ES13.6)
 7000 FORMAT(  T20,12('-'),T60,12('-'),
...
C-----FILE-WRITING ERROR
 ...
 9900 FORMAT(/,1X,'*** ERROR WRITING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &7X,'SPECIFIED FILE ACCESS: ',A,/
     &7X,'SPECIFIED FILE ACTION: ',A,/
     &2X,'-- STOP EXECUTION (GWM1OBJ2OT)')
      CALL USTOP(' ')
C
      END SUBROUTINE GWM1OBJ2OT
C
C
      END MODULE GWM1OBJ2