...
C-----------------------------------------------------------------------
      ...
      USE GWM1STC3, ONLY : GWM1STC3AR
      USE GWM1HDC3, ONLY : GWM1HDC3AR
...
C
 ...
 200      READ(INTEMP,'(A)',ERR=991,END=210)LINE ! READ A LINE
     ...
     &                  GWMOUT,INTEMP)          
          ...
          ELSEIF(KEYWORD.EQ.'OBJFNC')THEN        ! CORRESPONDING FILENAME
            FNAMEOPT(I,2)=FNAME
          ...
          ELSEIF(KEYWORD.EQ.'DIS'.OR.KEYWORD.EQ.'CONTROL')THEN
            ! THESE ARE FOR GWMPLL
          ELSE
...
C-----PROCESS DECISION VARIABLE FILE NAME(S) IN FNAMEOPT LOCATION 1
     ...
     &                  NBVAR,NDV,NGRIDS)
      ...
      ENDIF
C
C-----PROCESS OBJECTIVE FUNCTION FILE IN FNAMEOPT LOCATION 2
      IF(FNAMEOPT(1,2).NE.' ')THEN
        CALL GWM1OBJ3AR(FNAMEOPT(1,2),GWMOUT,NPER,REAL(PERLEN,DP),
     &                  NFVAR,NEVAR,NBVAR)
      ELSE
...
C-----INITIALIZE THE NUMBER OF VARIABLES CONSTRAINTS
      ...
      NRMC = 0
C
C-----PROCESS DECISION-VARIABLE CONSTRAINT FILE IN FNAMEOPT LOCATION 3
C     CHECK THAT EACH DECVAR FILE HAS A VARCON FILE
      J=0
      DO 500 I=1,NGRIDS
        IF((FNAMEOPT(I,1).NE.' ' .AND. FNAMEOPT(I,3).EQ.' ').OR.
     &     (FNAMEOPT(I,1).EQ.' ' .AND. FNAMEOPT(I,3).NE.' ')) J=1
  500 ENDDO    
      IF(J.GT.0)THEN
        ...
        CALL USTOP(' ')
      ENDIF
      CALL GWM1DCC3AR(FNAMEOPT(1:NGRIDS,3),GWMOUT,NFVAR,NEVAR,NBVAR,
     &                NV,NCON,NGRIDS)
C
C-----PROCESS SUMCON FOR PARENT IF KEYWORD PRESENT; ERROR IF KEYWORD IN CHILD
C     FILE(S) IN FNAMEOPT LOCATION 4
      IF(FNAMEOPT(1,4).NE.' ')THEN
        CALL GWM1SMC3AR(FNAMEOPT(1,4),GWMOUT,NFVAR,NEVAR,NBVAR,
     &                  NV,NCON)
      ENDIF
      IF(NGRIDS.GT.1)THEN
        DO 520 I=2,NGRIDS    
          IF(FNAMEOPT(I,4).NE.' ')THEN
  ...
  520   ENDDO
      ENDIF
C    
C-----PROCESS HEDCON FOR PARENT AND/OR CHILDREN; IN FNAMEOPT LOCATION 5
      CALL GWM1HDC3AR(FNAMEOPT(1:NGRIDS,5),GWMOUT,NPER,NV,NCON,
     &                NRMC,NGRIDS)
C
C-----PROCESS STRMCON FOR PARENT AND/OR CHILD; IN FNAMEOPT LOCATION 6
      CALL GWM1STC3AR(FNAMEOPT(1:NGRIDS,6),GWMOUT,NV,NCON,
     &                NRMC,NGRIDS)
C
C-----PROCESS SOLN FILE FOR PARENT; ERROR IF IN CHILD, DON'T STOP PROGRAM
      IF(FNAMEOPT(1,7).NE.' ')THEN
        CALL GWM1RMS3AR(FNAMEOPT(1,7),GWMOUT,NFVAR,NEVAR,NBVAR,
     &                  NDV,NV,NCON)
      ELSE
...
C
 ...
 1015 FORMAT(/,/1X,'OPENING GWM FILE FOR GWM1 -- GROUND-WATER ',
