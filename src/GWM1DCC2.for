...
C     VERSION: 20FEB2005
      IMPLICIT NONE
      PRIVATE
      PUBLIC::NLBR,GWM1DCC2AR,GWM1DCC2FM,GWM1DCC2OT
C
...
C
      CONTAINS
C***********************************************************************
      SUBROUTINE GWM1DCC2AR(FNAME,IOUT,NFVAR,NEVAR,NBVAR,NV,NCON)
C***********************************************************************
C     VERSION: 21JULY2005
C     PURPOSE: READ INPUT FROM THE DECISION-VARIABLES CONSTRAINT FILE
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO,SMALLEPS,BIGINF
      USE GWM1DCV2, ONLY : FVMIN,FVMAX,FVBASE,FVDIR,EVMIN,EVMAX,FVNAME,
     1                     EVNAME,BVLIST,BVNLIST
      CHARACTER(LEN=200),INTENT(IN)::FNAME
      INTEGER(I4B),INTENT(IN)::NFVAR,NEVAR,NBVAR
...
C
      ...
      END INTERFACE
C-----LOCAL VARIABLES
      INTEGER(I4B)::IPRN,INMS,INMF,I,J,JJ,INI
...
C-------PROCESS FVREF, THE REFERENCE FLOW RATES
        ...
        IF(TFVREF.LT.TPMN)WRITE(IOUT,3700,ERR=990)TFVNAME
...
C
 ...
 4100 FORMAT(1X,/1X,'PROGRAM STOPPED. WELL ',A10,' HAS A MINIMUM',
 ...
 5200 FORMAT(/,T2,'EXTERNAL VARIABLES:',/,T25,'MINIMUM',T40,
     1  'MAXIMUM',/,T3,'NUMBER',T12,'NAME',T26,'FLOW RATE',T41,
     2  'FLOW RATE',/,'---------------------------------------',
     3  '----------')
...
C-----FILE-OPENING ERROR
 ...
 9990 FORMAT(/,1X,'*** ERROR OPENING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &2X,'-- STOP EXECUTION (GWM1DCC2AR)')
      CALL USTOP(' ')
C
      END SUBROUTINE GWM1DCC2AR
C
C
C***********************************************************************
      SUBROUTINE GWM1DCC2FM(RSTRT,NSLK)
C***********************************************************************
C     VERSION: 22JULY2005
C     PURPOSE: PLACE CONSTRAINTS RELATED TO BINARY VARIABLES
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO,ONE
      USE GWM1DCV2, ONLY : NFVAR,NEVAR,NBVAR,FVMAX,FVMIN,EVMAX,EVMIN,
     &                     BVNLIST,BVLIST,EVNAME,FVNAME
      USE GWM1RMS2, ONLY : AMAT,RHS,
...
C
      RETURN
      END SUBROUTINE GWM1DCC2FM
C
C
C*************************************************************************
      SUBROUTINE GWM1DCC2OT(RSTRT)
C*************************************************************************
C     VERSION: 20FEB2005
C     PURPOSE: WRITE THE STATUS OF BINDING VARIABLE BOUND CONSTRAINTS 
C---------------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO,ONE
      USE GWM1DCV2, ONLY : NFVAR,NEVAR,NBVAR,FVNAME,FVMAX,FVMIN,
     &                     EVNAME,EVMAX,EVMIN,BVNAME,BVNLIST,BVLIST
      USE GWM1RMS2, ONLY : CST,BNDS,RHS,RANGENAME,NDV
...
C
     ...
     &    (ABS(CST(I)).EQ.FVMIN(I)))THEN  ! VARIABLE AT LOWER BOUND
          ...
          CALL GWM1BAS2CS(CTYPE,FVNAME(I),ZERO,0,0) 
...
C
     ...
     &    (ABS(CST(IEV)).EQ.EVMIN(I)))THEN  ! VARIABLE AT LOWER BOUND
          ...
          CALL GWM1BAS2CS(CTYPE,EVNAME(I),ZERO,0,0) 
