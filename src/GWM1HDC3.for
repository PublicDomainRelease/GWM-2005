...
C---------------------------------------------------------------------------
      USE GWM1BAS3, ONLY : BIGINF,IGETUNIT
      ...
      DO 200 I=NHB+NDD+1,NHB+NDD + NDF+NGD
        IF(IGRID.EQ.GRDLOCHDC(I))THEN                       ! CONSTRAINT ON GRID
          IF(KPER.EQ.HDCSP(I)) THEN                         ! ACTIVE THIS SP
            ...
            IF(STATE1.EQ.HDRY)DEWATER(IPERT)=.TRUE. ! CELL IS DEWATERED
            IF(STATE2.EQ.HDRY)DEWATER(IPERT)=.TRUE. ! CELL IS DEWATERED
...
C---- FOR HEAD BOUND CONSTRAINTS ASSIGN HEAD VALUE TO HDCSTATE
        DO 300 I=1,NHB+NDD   
          STATE1 = DEPVALS(I)
          IF(STATE1.EQ.HDRY)DEWATER(IPERT)=.TRUE.  ! CELL HAS DEWATERED
          HDCSTATE(I) = STATE1
  300   ENDDO
C
C---- FOR HEAD DIFFERENCE CONSTRAINTS ASSIGN HEAD DIFFERENCE TO HDCSTATE
        ...
        DO 400 I=NHB+NDD+1,NHB+NDD + NDF+NGD
          ...
          STATE2 = DEPVALS(K)
          IF(STATE1.EQ.HDRY)DEWATER(IPERT)=.TRUE.  ! CELL HAS DEWATERED
          IF(STATE2.EQ.HDRY)DEWATER(IPERT)=.TRUE.  ! CELL HAS DEWATERED
          HDCSTATE(I) = STATE1 - STATE2
  400   ENDDO
C
      ENDIF
