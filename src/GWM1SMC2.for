...
C     VERSION: 12AUG2007
      IMPLICIT NONE
      PRIVATE
      PUBLIC::GWM1SMC2AR,GWM1SMC2FM,GWM1SMC2OT
C
...
C
      CONTAINS
C***********************************************************************
      SUBROUTINE GWM1SMC2AR(FNAME,IOUT,NFVAR,NEVAR,NBVAR,NV,NCON)
C***********************************************************************
C   VERSION: 20FEB2005
C   PURPOSE: READ INPUT FROM THE GENERALIZED-SUMMATION CONSTRAINTS
C-----------------------------------------------------------------------
      USE GWM1BAS2, ONLY : ZERO
      USE GWM1DCV2, ONLY : FVNAME,EVNAME,BVNAME
...
C-----FILE-OPENING ERROR
 ...
 9990 FORMAT(/,1X,'*** ERROR OPENING FILE "',A,'" ON UNIT ',I5,/,
     ...
     &2X,'-- STOP EXECUTION (GWM1SMC2AR)')
      CALL USTOP(' ')
C
      END SUBROUTINE GWM1SMC2AR
C
C
C***********************************************************************
      SUBROUTINE GWM1SMC2FM(RSTRT,NSLK)
C***********************************************************************
...
C-----------------------------------------------------------------------
      ...
      USE GWM1RMS2, ONLY : CST,RHS,RHSIN,RANGENAME,NDV
      USE GWM1BAS2, ONLY : GWM1BAS2CS
      USE GWM1DCV2, ONLY : NFVAR,FVBASE,FVDIR
      INTEGER(I4B),INTENT(INOUT)::RSTRT
...
C-----WRITE STATUS FOR LINEAR PROGRAM OUTPUT
  ...
  200       ENDDO
            IF(ABS(LHS-RHSIN(ROW)).LT.SMALLEPS)THEN ! CONSTRAINT IS BINDING
              CTYPE = 'Summation'
              CALL GWM1BAS2CS(CTYPE,SMCNAME(ISMC),ZERO,0,0) 
            ENDIF
          ELSEIF(ABS(RHS(ROW)).GT.ZERO)THEN        ! DUAL VARIABLE IS NON-ZERO 
C                                                  ! CONSTRAINT IS BINDING         
            CTYPE = 'Summation'
            CALL GWM1BAS2CS(CTYPE,SMCNAME(ISMC),RHS(ROW),0,0) 
          ENDIF
  210   ENDDO
